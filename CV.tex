...
\setname{}{Josh \bf{Day}}
\setaddress{2191 El Sereno Ave. -- Altadena, CA}
...
% Experience
\section{Experience}
	\datedexperience{Columbus Technologies \& Services}{May 2021 -- Present}
	\explanationwithlocation{Mechanical Engineer III (TTAPS, NASA Jet Propulsion Laboratory)}{Pasadena, CA}
	\explanationdetail{%
		\begin{itemize}[label=\textcolor{red}{$\circ$}, leftmargin=*]
			\setlength\itemsep{-0.05em}
			\item Designed and constructed prototype securement mechanism for Mars Sample Return (MSR) orbiting sample (OS) canister and oversaw test campaign to characterize performance which led to immediate design refinements.
	...
	\explanationdetail{%
		\begin{itemize}[label=\textcolor{red}{$\circ$}, leftmargin=*]
			\setlength\itemsep{-0.05em}
			\item Defined requirements for and designed, simulated, and constructed a small satellite propulsion system prototype and custom-built a 3 degree-of-freedom air bearing platform to simulate microgravity for propulsion system testing.
			\item Architectured and integrated propulsion system control hardware and developed full stack human-machine interface for real-time wireless control and sensor feedback using Arduino and Raspberry Pi programmed in C++ and JavaScript.
	...
	\explanationdetail{%
		\begin{itemize}[label=\textcolor{orange}{$\circ$}, leftmargin=*]
			\setlength\itemsep{-0.05em}
			\vspace{-0.9em}
			\item \emph{Engineering Design \& Communications} (Lead) \qquad \textcolor{red}{$\circ$} \emph{Measurement Systems} \qquad \textcolor{red}{$\circ$} \emph{Manufacturing Processes}
		\end{itemize}
	}
...
\section{Technical Skills}
	...
	\explanationdetail{\footnotesize \textsc{%
		\begin{tabular}{ r l l }
			...
			\textbf{CAD/CAM Software}				& Siemens NX \& NX NASTRAN \cpshalf SolidWorks \cpshalf Slic3r \cpshalf Autodesk EAGLE \cpshalf Microsoft Visio  	\\
			\textbf{Development Tools}				& Siemens TeamCenter \cpshalf Git \cpshalf Linux / WSL \cpshalf VS Code	\cpshalf Drupal														\\
			\textbf{Programming Languages}			& Python \cpshalf Arduino (C++) \cpshalf MATLAB \cpshalf JavaScript \cpshalf HTML \cpshalf CSS 			\\
			\textbf{Manufacturing Experience}		& CNC Machining \cpshalf 3D Printing \cpshalf PCB Design \cpshalf Wiring Harness Fabrication \\
		\end{tabular}
		}
	}
